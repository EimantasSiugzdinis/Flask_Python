...
def get_log():
    ...
    try:
        ...
        return (
            jsonify(
                {"error": f"An error occurred while reading the log file: {str(e)}"}
            ),
            500,
        )
...
def save_log():
    ...
    try:
        ...
        if e.errno == errno.ENOSPC:
            return (
                jsonify({"error": "No space left on device to write the log file"}),
                507,
            )
        else:
            return (
                jsonify(
                    {
                        "error": f"An OS error occurred while writing to the log file: {str(e)}"
                    }
                ),
                500,
            )
        ...
        return (
            jsonify(
                {"error": f"An error occurred while writing to the log file: {str(e)}"}
            ),
            500,
        )
...
def parse_log_contents(log_data):
    ...
    measurements_values = re.findall(
        r"\[(.*?)\] MEASUREMENT: (\w+) concentration - (\d+\.?\d*)", log_data
    )
    ...
    for timestamp, gas, value in measurements_values:
        try:
            ...
            print(
                "Failed to convert the value to an integer."
            )  # how to handle this? Should I print in the response or in logs? now that I write it... I guess we need an error block...
            continue
